...
export default function App() {
  ...
  // Active platform tab: 'dashboard' (analizador XML), 'accounts' (administrador de cuentas), 'bank-statements' (analizador estado de cuenta) or 'sat-ws' (conexión SAT-GO)
  const [activeTab, setActiveTab] = useState<'dashboard' | 'accounts' | 'bank-statements' | 'sat-ws'>('dashboard');
  ...
  const handleImportXMLs = (xmls: { text: string; fileName: string }[]) => {
    const parsedArray: ParsedCFDI[] = [];
    for (const xml of xmls) {
      try {
        const parsed = parseXMLData(xml.text, xml.fileName);
        parsedArray.push(parsed);
      } catch (err) {
        console.error(`Error de parseo para el archivo importado ${xml.fileName}:`, err);
      }
    }
    setUploadedFiles(prev => {
      // Overwrite if same file is re-added
